-- ============================================================
-- 生产环境修复脚本：云存储配置打开提示无权限
-- 适用：管理后台「系统设置 → 云存储配置」Tab 显示 403「暂无访问权限」
-- 原因：云存储配置权限（system:settings:cloud:list/update）仅写入 schema.sql（新库），
--       存量生产库未包含这两条权限，super 角色拿不到 → 前端 hasAuth 校验失败
-- 说明：
--   1. 执行前请先备份数据库（宝塔 → 数据库 → 备份）
--   2. 所有语句均幂等，重复执行不会产生副作用
--   3. 全部选中后一次性执行即可；或用命令行导入：
--      mysql -u<用户> -p<密码> family_genealogy < fix-prod-cloud-storage-perms.sql
--   4. 执行完成后：pm2 restart family-genealogy-server，并强刷浏览器（Ctrl+F5）
-- ============================================================

-- ------------------------------------------------------------
-- ① 补充云存储配置权限码（缺失会导致接口 403 无权访问）
-- ------------------------------------------------------------
...
WHERE r.`code` = 'super'
  AND p.`code` LIKE 'system:settings:cloud:%';
...
FROM `sys_permission`
WHERE `code` LIKE 'system:settings:cloud:%';
